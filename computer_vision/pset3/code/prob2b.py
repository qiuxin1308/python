...
def ransac(points, K, N, eps):
    ...
    arr = list(range(n))
    for i in range(N):
        index = np.random.choice(arr, K, replace=False)
        theL = fitLines(points[index,:])
        px, py = points[:,0], points[:,1]
        pm, pb = theL
        err = np.power((py-pm*px-pb),2)
        inlier_set = points[np.where(err < eps)]
        if len(inlier_set) > len(largest_set):
            largest_set = inlier_set.copy()
    newL = fitLines(largest_set)
...
def fitLines(ps):
    ...
    diff_ex = x - np.mean(x)
    diff_ey = y - np.mean(y)
    m = np.sum(diff_ex * diff_ey) / np.sum(diff_ex * diff_ex)
    b = (np.sum(y) - np.sum(x) * m) / n
    L = (m, b)
